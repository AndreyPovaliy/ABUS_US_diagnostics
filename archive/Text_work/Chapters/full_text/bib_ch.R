...
## подключение фалов ------------------------------------------------------------
take_f_txt_keys <- read.csv("archive/Text_work/Chapters/full_text/Abus_full.Rmd",header = F,sep = '\n')


bib_file_path <- "archive/Text_work/Chapters/full_text/references.bib"

file_content <- readLines("archive/Text_work/Chapters/full_text/Abus_full.Rmd", warn = FALSE)
...
## неправильный формат
# df_ch_format <- tibble(
#   wrong = bib_dataframe$BIBTEXKEY[!(str_detect(bib_dataframe$BIBTEXKEY, "^[a-z]+[0-9][0-9][0-9][0-9]$"))],
#   righrt = c("kotsianoshermle2009" , 
#              "medicaladvisorysecretariat2007", 
#              "tossasmilligan2019", 
#              "giger2016"))
# 
# 
# for (i in 1:nrow(df_ch_format)){
# 
#   bib_repl <- readLines("archive/Text_work/Chapters/full_text/references.bib")
#   modified_bib <- gsub(df_ch_format$wrong[i], df_ch_format$righrt[i], bib_repl)
#   writeLines(modified_bib, "archive/Text_work/Chapters/full_text/references.bib")
# }
