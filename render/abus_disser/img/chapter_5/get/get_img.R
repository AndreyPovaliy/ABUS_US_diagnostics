...
png("/Users/andreypovaliy/Yandex.Disk.localized/Documents/science/ABUS_US_diagnostics/render/abus_disser/img/chapter_5/ch_5_fig_n24.png", width = 1600, height = 1600, res = 200)

# Настройка области для 4 графиков (2 ряда, 2 колонки)
par(mfrow = c(1, 2),          # 2 ряда, 2 колонки
    mar = c(5, 5, 4, 2),      # Отступы: снизу, слева, сверху, справа
    oma = c(0, 0, 2, 0))      # Внешние отступы для общего заголовка

# График 1 (Группа 1, УЗИ)
roc(dfXlsxGr4$mmg_is_calc, dfXlsxGr4$us_probabilityCalc,
    smoothed = TRUE,
    ci = TRUE, 
    ci.alpha = 0.9, 
    stratified = FALSE,
...
roc(dfXlsxGr4$mmg_is_calc, dfXlsxGr4$abus_probabilityCalc,
    smoothed = TRUE,
    ci = TRUE, 
    ci.alpha = 0.9, 
    stratified = FALSE,
...
cat("Файл сохранен: ch_5_fig_n24.png\n")
